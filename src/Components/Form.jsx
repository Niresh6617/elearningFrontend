...
export const Form = ({ particulardata, onreload }) => {
    ...
    useEffect(() => {
      ...
      const submitingdata = async (e) => {
        e.preventDefault()
        if (particulardata) {
          await axios.put(`https://elearningbackend-v4hk.onrender.com/put/course/${particulardata._id}`, { title, description, duration })
          setTitle('')
          setDescription('')
          setDuration('')
          alert('Course Updated Successfully')
        } else {
          await axios.post('https://elearningbackend-v4hk.onrender.com/post/course', { title, description, duration })
          setTitle('')
          setDescription('')
          setDuration('')
